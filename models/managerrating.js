...
const ratingSchema = mongoose.Schema({
    ...
    RategivenBy:{ type: String, required: true },
});

// Define compound index for EmployeeId and selectedDate
ratingSchema.index({ EmployeeId: 1, selectedDate: 1 ,Department: 1}, { unique: true });

// Middleware to check if the selectedDate is unique for the given EmployeeId
ratingSchema.pre('save', async function (next) {
    try {
        const existingRating = await this.constructor.findOne({ EmployeeId: this.EmployeeId,Department:this.Department, selectedDate: this.selectedDate });
        if (existingRating) {
...
module.exports = mongoose.model('ManagerRating', ratingSchema);
