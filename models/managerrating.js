...
ratingSchema.index({ EmployeeId: 1, selectedDate: 1 }, { unique: true });

// Middleware to check if the selectedDate is unique for the given EmployeeId
ratingSchema.pre('save', async function (next) {
    try {
        const existingRating = await this.constructor.findOne({ EmployeeId: this.EmployeeId, selectedDate: this.selectedDate });
        if (existingRating) {
